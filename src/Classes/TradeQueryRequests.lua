	...
	table.insert(self.requestQueue["fetch"], {
		...
		callback = function(response, errMsg)
			...
			for _, trade_entry in pairs(response.result) do
				...
				-- ConPrintf("Fetched item [%s]: whisper_token: %s, hideout_token: %s", itemData.id, tostring(itemData.whisper_token), tostring(itemData.hideout_token))
...
---Fetch item details for a single itemHash to refresh its data (e.g., tokens)
---@param itemId string
---@param queryId string
---@param callback fun(item:table, errMsg:string)
function TradeQueryRequestsClass:FetchSingleItem(itemId, queryId, callback)
	-- ConPrintf("Fetching single item data for ID: %s", itemId)
	local fetch_url = self.hostName .. "api/trade2/fetch/"..itemId.."?query="..queryId
	self:FetchResultBlock(fetch_url, function(itemBlock, errMsg)
		if errMsg then
			return callback(nil, errMsg)
		end
		if itemBlock and #itemBlock > 0 then
			local singleItemData = itemBlock[1]
			-- ConPrintf("Successfully fetched single item. hideout_token: %s, whisper_token: %s", tostring(singleItemData.hideout_token), tostring(singleItemData.whisper_token))
			callback(singleItemData)
		else
			callback(nil, "No item data found in single fetch response")
		end
	end)
end
