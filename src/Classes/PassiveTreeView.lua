...
function PassiveTreeViewClass:AddNodeTooltip(tooltip, node, build, incSmallPassiveSkillEffect)
	...
	local function describeAndAddStats(stats, statSet, prefix, quality)
		local descriptions, lineMap = data.describeStats(stats, statSet.statDescriptionScope, quality)
		for i, line in ipairs(descriptions) do
			line = line
			local source = (statSet and statSet.statMap[lineMap[line]]) or data.skillStatMap[lineMap[line]]
	...
	local function addStatSetInfo(gemInstance, grantedEffect, statSet, noLabel, index)
		local fontSizeBig = main.showFlavourText and 18 or 16
		...
		local statsLevel1 = calcLib.buildSkillInstanceStats(copiedInstance, grantedEffect, statSet)

		for statName, min in pairs(statsLevel1) do
			local max = statsLevel20[statName]
			-- the stat describer will ignore the range when these are both equal
			statsLevel1[statName] = {min = min, max = max}
		end

		describeAndAddStats(statsLevel1, statSet)
	end

	function addGrantedEffectInfo(gemInstance, grantedEffect, addReq)
		local fontSizeBig = main.showFlavourText and 18 or 16

		-- similar to stat descriptors, formats to either (x-y) or x, depending on
		-- if the value scales with level or not
		local function formatRangeOrValue(keyName, formatter, add, div, mul)
			local lv1Value = grantedEffect.levels[1][keyName]
			local lv20Value = grantedEffect.levels[20][keyName]
			if not lv1Value then
				return nil
			end
			local numFormat = formatter or "%d"
			if lv1Value == lv20Value then
				return string.format("%d", (mul or 1) * (lv1Value + (add or 0)) / (div or 1))
			end
			local rangeFormat = "("..numFormat.."-"..numFormat..")"
			return string.format(rangeFormat, (mul or 1) * (lv1Value + (add or 0)) / (div or 1),
				(mul or 1) * (lv20Value + (add or 0)) / (div or 1))
		end
		local lv1Stats = grantedEffect.levels[1]
		if gemInstance.skillMinion then
			if gemInstance.nameSpec:match("^Spectre:") then
				grantedEffect.levels[1].spiritReservationFlat = data.spectres[gemInstance.skillMinion]
					.spectreReservation
			elseif gemInstance.nameSpec:match("^Companion:") then
				grantedEffect.levels[1].spiritReservationPercent = data.spectres[gemInstance.skillMinion]
		...
		local cost
		-- for each resource stat that the skill has, we add a formatted value
		-- with either the number, or (x-y) range of numbers to show level
		-- scaling
		for _, res in ipairs(data.costs) do
			if lv1Stats.cost and lv1Stats.cost[res.Resource] then
				if cost then
					cost = cost..", "
				else
					cost = ""
				...
				local lv1Val = round(lv1Stats.cost[res.Resource] / res.Divisor, 2)
				local lv20Val = round(grantedEffect.levels[20].cost[res.Resource] / res.Divisor, 2)
				
				local valString
				if lv1Val == lv20Val then
					valString = string.format("%d", lv1Val)
				else
					valString = string.format("(%d-%d)", lv1Val, lv20Val)
				end

				cost = cost..res.ResourceString:gsub("{0}", valString)
			end
		end
		if cost then
	...
	function addGemTooltip(gemInstance)
		...
		for idx, statSet in ipairs(grantedEffect.statSets) do
			addStatSetInfo(gemInstance, grantedEffect, statSet, nil, idx)
		end
		...
		for idx, additional in ipairs(additionalEffects or {}) do
			if not additional.support then
				...
				for _, statSet in ipairs(additional.statSets) do
					addStatSetInfo(gemInstance, additional, statSet, true, idx)
				end
			end
		end
		...
		if grantedEffect.qualityStats then
			...
			for _, stat in ipairs(grantedEffect.qualityStats) do
				if stat[1] and stat[2] then
					qualityStats[stat[1]] = { min = 0, max = 20 * stat[2] }
				end
			end
			-- note that this is not technically correct. quality is not
			-- currently exported correctly in pob, and qualities should
			-- actually be linked to stat sets, as they might not apply to all
			-- stat sets of the skill. after this is fixed, these should be
			-- exported in addStatSetInfo.
			describeAndAddStats(qualityStats, grantedEffect.statSets[1], "Quality Effect: ", true)
		end
