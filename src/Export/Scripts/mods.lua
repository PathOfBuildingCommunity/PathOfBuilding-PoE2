...
local function writeMods(outName, condFunc)
		...
		if condFunc(mod) then
			...
			if #orders > 0 then
				...
				for _, tag in ipairs(mod.SpawnTags) do
					table.insert(spawnTags, getSpawnWeightTag(mod, tag))
				end
				local weightMultiplierKey = {}
				for _, tag in ipairs(mod.GenerationWeightTags) do
					table.insert(weightMultiplierKey, tag.Id)
				end
				local tags = {}
				...
				local entry = {
					...
					group = mod.Type.Id,
					weightKey = spawnTags,
					weightVal = mod.SpawnWeight,
					weightMultiplierKey = #weightMultiplierKey > 0 and weightMultiplierKey or nil,
					weightMultiplierVal = #weightMultiplierKey > 0 and mod.GenerationWeightValues or nil,
	...
	utils.saveTableToFile(outName, out, "Item modifier data generated by mods.lua")
end
