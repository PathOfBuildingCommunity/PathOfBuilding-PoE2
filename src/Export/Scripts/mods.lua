...
writeMods("../Data/ModItem.lua", function(mod)
	return mod.Domain == Domains.GenericMod and
		...
		and not mod.Id:match("^HandWraps")
end)
...
writeMods("../Data/ModFistsOfStone.lua", function(mod)
	return mod.Id:match("^HandWraps")
end)
writeMods("../Data/ModVeiled.lua", function(mod)
	return mod.Domain == Domains.Veiled and not mod.Id:match("Map")
end)

local function writeStonefistMap(outName)
	local targetById = { }
	local targetsByName = { }
	local sources = { }
	for mod in dat("Mods"):Rows() do
		if mod.Id:match("^HandWraps") then
			targetById[mod.Id] = mod
			local name = mod.Name or ""
			targetsByName[name] = targetsByName[name] or { }
			table.insert(targetsByName[name], mod)
		else
			sources[#sources + 1] = mod
		end
	end

	local keys, map = { }, { }
	for _, source in ipairs(sources) do
		local bareId = source.Id:gsub("_+$", "")
		local target = targetById["HandWraps" .. source.Id] or targetById["HandWraps" .. bareId]
		if not target then
			local named = targetsByName[source.Name or ""]
			if named and #named == 1 and (source.Name or "") ~= "" then
				target = named[1]
			end
		end
		if target then
			if (source.Name or "") ~= (target.Name or "") then
				ConPrintf("Stonefist name mismatch: %s ('%s') -> %s ('%s')",
					source.Id, tostring(source.Name), target.Id, tostring(target.Name))
			end
			if not map[source.Id] then
				map[source.Id] = target.Id
				keys[#keys + 1] = source.Id
			end
		end
	end
	...
	out:write('}')
	out:close()
	print(string.format("Stonefist map exported (%d entries).", #keys))
end
writeStonefistMap("../Data/DataStonefistMap.lua")
