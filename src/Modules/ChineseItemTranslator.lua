...
local LABELS = {
	...
	["限制"] = "Limited to",
	["半径"] = "Radius",
	["范围"] = "Radius",   -- jewel radius property (JewelRadiusLabel)
...
-- Jewel radius sizes (value of a "范围:" line).
local RADIUS = {
	["极小"] = "Very Small", ["小"] = "Small", ["小型"] = "Small",
	["中小"] = "Medium-Small", ["中"] = "Medium", ["中型"] = "Medium",
	["中大"] = "Medium-Large", ["大"] = "Large", ["大型"] = "Large",
	["极大"] = "Very Large", ["巨大"] = "Massive",
}
...
local function translateValue(enLabel, val)
	...
	if enLabel == "Radius" then return RADIUS[val] or val end
...
-- Strip the trade-site "[Key|Display]" / "[Display]" markup down to the Display
-- text. The game client removes this before showing an item, but the trade
-- website keeps it in its Ctrl+C text — so our datamined (stripped) templates
-- only match after we strip it here too. e.g. "[EnergyShield|能量护盾]提高 113%"
-- -> "能量护盾提高 113%".
...
local function translateLine(line)
	...
	-- "羁绊: <mod>" — the trade format's Bonded (ShamanOnlyMods) soul-core prefix.
	local bondedMod = line:match("^羁绊:%s*(.+)$")
	if bondedMod then
		local m = translateModLine(bondedMod)
		if m then return "Bonded: " .. m end
	end
	...
	-- Bare item-class line (trade format lists the class on its own line after the
	-- base, e.g. "头盔"). PoB derives the class from the base, so drop it to avoid a
	-- spurious unrecognised line. (The game format uses a "物品类别:" label instead.)
	-- Checked before uniques because Words.Text2 also contains class words.
	if data.classes[line] then return nil end
	...
	-- Lost/Time-Lost jewel radius mods: "范围内的核心天赋同时提供 <sub-stat>" — a wrapper
	-- + a separate sub-stat, not a single template. PoB sources the actual mod from the
	-- affix name, so this is for readability; translate the sub-stat and re-wrap it.
	local radiusSub = line:match("^范围内的核心天赋同时提供%s*(.+)$")
	if radiusSub then
		local en = translateModLine(radiusSub)
		if en then return "Notable Passive Skills in Radius also grant " .. en end
	end
