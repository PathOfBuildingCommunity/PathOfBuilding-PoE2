...
function main:OnFrame()
	...
	if self.toastMessages[1] then
		...
		if self.toastMode then
			SetDrawStyle('toast_border')
			DrawImage(nil, 0, self.screenH - self.mainBarHeight, 312, self.mainBarHeight)
			SetDrawStyle('toast_background')
			DrawImage(nil, 0, self.screenH - self.mainBarHeight + 4, 308, self.mainBarHeight - 4)
			SetDrawStyle('text_toast_heading')
			StyledDrawString(4, self.screenH - self.mainBarHeight + 8, "LEFT", 20, 'text_toast_heading', self.toastMessages[1]:gsub("\n.*",""))
			SetDrawStyle('text_toast')
			StyledDrawString(4, self.screenH - self.mainBarHeight + 28, "LEFT", 16, 'text_toast', self.toastMessages[1]:gsub("^[^\n]*\n?",""))
		end
	end

	-- Draw main controls (bottom left corner)
	SetDrawStyle('main_control_border')
	DrawImage(nil, 0, self.screenH - 58, 312, 58)
	SetDrawStyle('main_control_background')
	...
	for _, node in ipairs(setXML[1]) do
		if type(node) == "table" then
			...
			elseif node.elem == "Misc" then
				...
				if node.attrib.activeTheme then
					self.activeTheme = node.attrib.activeTheme
					SetActiveTheme(self.activeTheme)
				end
			end
		end
	end
...
function main:OpenOptionsPopup(savedState)
	...
	local function drawSectionHeader(id, title, omitHorizontalLine)
		local headerBGstyle = 'rectangle_outline_border'
		controls["section-"..id .. "-bg"] = new("RectangleOutlineControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + scrollBarWidth + 8, currentY, columnWidth - (scrollBarWidth * 2) - 17, 26 }, headerBGstyle, 1)
		nextRow(.2)
		controls["section-"..id .. "-label"] = new("LabelControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + columnWidth / 2 - 60, currentY, 0, 16 }, GetStyleColor('text_heading') .. title)
		nextRow(1.5)
	end
	...
	controls.connectionProtocolLabel = new("LabelControl", { "RIGHT", controls.connectionProtocol, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Connection Protocol:")
	...
	controls.proxyLabel = new("LabelControl", { "RIGHT", controls.proxyType, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Proxy server:")
	...
	controls.dpiScaleOverrideLabel = new("LabelControl", { "RIGHT", controls.dpiScaleOverride, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "UI scaling override:")
	...
	controls.buildPathLabel = new("LabelControl", { "RIGHT", controls.buildPath, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Build save path:")
	...
	controls.nodePowerThemeLabel = new("LabelControl", { "RIGHT", controls.nodePowerTheme, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Node Power colours:")
	...
	controls.colorHighlightLabel = new("LabelControl", { "RIGHT", controls.colorHighlight, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Hex colour for highlight nodes:")
	...
	nextRow()
	controls.betaTest = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Opt-in to weekly beta test builds:", function(state)
		self.betaTest = state
	end)

	nextRow()
	controls.edgeSearchHighlight = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20}, "Show search circles at viewport edge", function(state)
		self.edgeSearchHighlight = state
	end)

	nextRow()
	controls.showFlavourText = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Styled Tooltips with Flavour Text:", function(state)
	...
	nextRow()
	controls.showAnimations = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Show Animations:", function(state)
		self.showAnimations = state
	end)
	
	nextRow()
	controls.showAllItemAffixes = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Show all item affixes sliders:", function(state)
	...
	nextRow()
	controls.theme = new("DropDownControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 150, 18 }, {
		{ label = "Classic", themeIdentifier = "classic" },
		{ label = "Dark", themeIdentifier = "dark" },
	}, function(index, value)
		self.activeTheme = value.themeIdentifier
	...
	controls.showThousandsSeparators = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT"}, { currentX + defaultLabelPlacementX, currentY, 20 }, "Show thousands separators:", function(state)
	...
	nextRow()
	controls.titlebarName = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Show build name in window title:", function(state)
	...
	controls.defaultGemQualityLabel = new("LabelControl", { "RIGHT", controls.defaultGemQuality, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Default gem quality:")
	...
	controls.defaultItemQualityLabel = new("LabelControl", { "RIGHT", controls.defaultItemQuality, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Default item quality:")
	...
	controls.defaultCharLevelLabel = new("LabelControl", { "RIGHT", controls.defaultCharLevel, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "Default character level:")
	...
	controls.defaultItemAffixQualityLabel = new("LabelControl", { "RIGHT", controls.defaultItemAffixQualitySlider, "LEFT" }, { defaultLabelSpacingPx, 0, 92, 16 }, "Default item affix quality:")
	...
	nextRow()
	controls.showWarnings = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Show build warnings:", function(state)
		self.showWarnings = state
	end)
	controls.showWarnings.state = self.showWarnings

	nextRow()
	controls.slotOnlyTooltips = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Show tooltips only for affected slots:", function(state)
	...
	nextRow()
	controls.migrateAugments = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Copy augments onto display item:", function(state)
	...
	nextRow()
	controls.notSupportedModTooltips = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Show tooltip for unsupported mods :", function(state)
	...
	nextRow()
	controls.invertSliderScrollDirection = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Invert slider scroll direction:", function(state)
	...
	if launch.devMode then
		nextRow()
		controls.disableDevAutoSave = new("CheckBoxControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "Disable Dev AutoSave:", function(state)
	...
	controls.cancel = new("ButtonControl", { "BOTTOM", nil, "BOTTOM" }, {45, -10, 80, 20}, "Cancel", function()
		...
		SetDPIScaleOverridePercent(self.dpiScaleOverridePercent)
		self.activeTheme = savedState.activeTheme
		SetActiveTheme(self.activeTheme)
		main:ClosePopup()
	end)
...
function main:OpenMessagePopup(title, msg)
	...
	return self:OpenPopup(m_max(StyledDrawStringWidth(16, 'text_label', msg) + 30, 190), 70 + numMsgLines * 16, title, controls, "close")
end
...
function main:OpenConfirmPopup(title, msg, confirmLabel, onConfirm, extraLabel, onExtra)
	...
	local confirmWidth = m_max(80, StyledDrawStringWidth(16, 'text_button', confirmLabel) + 10)
	
	if extraLabel and onExtra then
		-- Three button layout: Continue (left), Connect Path (center), Cancel (right)
		local extraWidth = m_max(80, StyledDrawStringWidth(16, 'text_button', extraLabel) + 10)
		...
		return self:OpenPopup(m_max(StyledDrawStringWidth(16, 'text_label', msg) + 30, totalWidth + 40), 70 + numMsgLines * 16, title, controls, "confirm")
	else
		...
		return self:OpenPopup(m_max(StyledDrawStringWidth(16, 'text_label', msg) + 30, 190), 70 + numMsgLines * 16, title, controls, "confirm")
	end
end
